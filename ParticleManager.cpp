...
//unsigned short Object3d::indices[indexCount/*planeCount * 3*/];
...
void ParticleManager::UpdateViewMatrix()
{
	...
	// 視点座標に-1を掛けた座標
	XMVECTOR removeEyePosition = XMVectorNegate(eyePosition);
	// カメラの位置からワールド原点へのベクトル (カメラ座標系)
	XMVECTOR tX = XMVector3Dot(cameraAxisX, removeEyePosition);
	XMVECTOR tY = XMVector3Dot(cameraAxisY, removeEyePosition);
	XMVECTOR tZ = XMVector3Dot(cameraAxisZ, removeEyePosition);
	// ひとつのベクトルにまとめる
...
#pragma region 全方向ビルボード行列の計算
	...
	matBillboard.r[2] = XMVectorSet(0, 0, 0, 1);
#pragma region
